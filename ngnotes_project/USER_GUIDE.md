...
- **Add image block** — upload a photo (a whiteboard, a hand-drawn diagram, an engineering drawing, a screenshot). Click **Analyze image** to have a vision-capable model describe it; the description is added as source material alongside your notes. Your model needs vision support (check `ollama list` — both `qwen3.6` and `gemma4` include it, though **`gemma4` is the preferred model for vision specifically** — it tends to give more accurate image descriptions). It's a good idea to have both models installed if you'll be using images: `qwen3.6` for report generation, `gemma4` for image analysis.
