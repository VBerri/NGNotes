...
- **Add image block** — upload a photo (a whiteboard, a hand-drawn diagram, an engineering drawing, a screenshot). Click **Analyze image** to have a vision-capable model describe it; the description is added as source material alongside your notes. Both `qwen3.6` and `gemma4` handle vision well (check `ollama list` to confirm what you have) — they're the two models we've actually tested thoroughly for this whole pipeline and both hold up equally well, so stick to one of those two rather than an untested model. Having both installed is convenient; if your hardware can handle it, the larger variants (`qwen3.6:27b`/`:35b`, `gemma4:26b`/`:31b`) give better quality.
